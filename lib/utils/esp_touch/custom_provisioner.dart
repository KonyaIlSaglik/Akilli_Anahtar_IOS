...
import 'package:akilli_anahtar/utils/esp_touch/custom_esp_touch.dart';
import 'package:akilli_anahtar/utils/esp_touch/custom_esp_touch_v2.dart';
import 'package:akilli_anahtar/utils/esp_touch/custom_protocol.dart';
...
/// CustomProvisioner
class CustomProvisioner {
  /// Protocol
  final CustomProtocol _protocol;

  /// Provisioning isolate
  Isolate? _isolate;

  /// CustomProvisioner running indicator
  bool get running => _isolate != null;

  /// Responses stream controller
  final _streamCtrl = StreamController<CustomProvisioningResponse>();

  /// Constructor for new EspProvisioner with desired [protocol]
  CustomProvisioner._(this._protocol);

  /// CustomProvisioner with [EspTouchV2] protocol
  factory CustomProvisioner.espTouchV2() {
    return CustomProvisioner._(CustomEspTouchV2());
  }

  /// CustomProvisioner with [EspTouch] protocol
  factory CustomProvisioner.espTouch() {
    return CustomProvisioner._(CustomEspTouch());
  }

  /// Subscribe to provisioner events stream
  StreamSubscription<CustomProvisioningResponse> listen(
      void Function(CustomProvisioningResponse)? onData,
      {Function? onError,
  ...
  Future<void> start(CustomProvisioningRequest request) async {
    // if (running) {
  ...
  static void _startProvisioning(_EspWorker worker, SendPort sPort) async {
    ...
    final tmrDuration = Duration(milliseconds: 10);
    Timer.periodic(
...
class _EspWorker {
  final CustomProvisioningRequest request;
  final Logger logger;
  final CustomProtocol protocol;
...
class _EspWorkerEvent {
  ...
  factory _EspWorkerEvent.result(CustomProvisioningResponse result) {

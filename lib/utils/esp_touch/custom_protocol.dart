...
abstract class CustomProtocol {
  ...
  late final CustomProvisioningRequest request;
  ...
  final _responsesList = <CustomProvisioningResponse>[];
  ...
  void install(RawDatagramSocket socket, int portIndex,
      CustomProvisioningRequest request, Logger logger) {
    _socket = socket;
  ...
  CustomProvisioningResponse? findResponse(
  ...
  CustomProvisioningResponse addResponse(CustomProvisioningResponse response) {
  ...
  int send(List<int> buffer) {
    ...
    //   _socket.send(
    //       buffer,
    //       InternetAddress.fromRawAddress(Uint8List.fromList([234, i, i, i])),
    //       devicePort);
    // }

    _socket.send(buffer, request.address, devicePort);
    return 0;
  }

...
class MyProvisioner {
  ...
  final _streamCtrl = StreamController<MyProvisioningResponse>();

  /// Constructor for new EspProvisioner with desired [protocol]
  MyProvisioner._(this._protocol);

  // /// Provisioner with [EspTouchV2] protocol
  // factory MyProvisioner.espTouchV2() {
  //   return MyProvisioner._(EspTouchV2());
  // }

  /// Provisioner with [EspTouch] protocol
  factory MyProvisioner.espTouch() {
    return MyProvisioner._(MyEspTouch());
  }

  /// Subscribe to provisioner events stream
  StreamSubscription<MyProvisioningResponse> listen(
      void Function(MyProvisioningResponse)? onData,
      {Function? onError,
  ...
  Future<void> start(MyProvisioningRequest request) async {
    if (running) {
...
class _EspWorker {
  final MyProvisioningRequest request;
  final Logger logger;
  final MyProtocol protocol;
...
class _EspWorkerEvent {
  ...
  factory _EspWorkerEvent.result(MyProvisioningResponse result) {
